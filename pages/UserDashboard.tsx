...
const UserDashboard: React.FC<UserDashboardProps> = ({ user, onBack, properties, onPropertySelect, onLogout }) => {
  ...
  // Mock de Favoritos (Para demonstração, vamos pegar alguns imóveis aleatórios que não são do usuário)
  const myFavorites = properties.filter(p => p.ownerId !== user.id).slice(0, 3);
            ...
            <div className="lg:col-span-1">
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 sticky top-24">
                    <div className="flex flex-col items-center mb-6">
                        <div className="size-24 rounded-full bg-slate-200 bg-cover bg-center border-4 border-slate-50 mb-3" style={{ backgroundImage: `url("${user.avatar}")` }}></div>
                        <h2 className="text-xl font-bold text-slate-900">{user.name}</h2>
                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide mt-2 ${user.role === 'owner' ? 'bg-indigo-100 text-indigo-700' : 'bg-emerald-100 text-emerald-700'}`}>
                    ...
                    <nav className="flex flex-col gap-2">
                        <button 
                            ...
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'profile' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
                        ...
                        <button 
                            ...
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'favorites' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
                        ...
                        {user.role === 'owner' && (
                            <button 
                                ...
                                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${activeTab === 'listings' ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-600 hover:bg-slate-50'}`}
                            >
                                <span className="material-symbols-outlined">real_estate_agent</span> Meus Imóveis
                            </button>
                        )}
                    </nav>
                </div>
